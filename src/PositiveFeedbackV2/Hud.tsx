...
import { ACCENT, DIM_TEXT, HIGHLIGHTS, LINE_INACTIVE, MONTHS_FILL_END, MONTHS_FILL_START } from "./layout";
...
// Only beats 1-6 use the generic diagram + caption treatment; 7-9 render
// their own dedicated full-screen scenes (see index.tsx).
...
// word timestamps (not the whole beat's paragraph held statically) — this is
// both more accurate (always an exact, complete sentence, never a fragment)
// and reads as the screen actively following the narration rather than
// sitting still under it. Exactly one phrase per beat gets accent emphasis;
...
export const MonthsBar: React.FC = () => {
  ...
  const barOpacity = interpolate(
    frame,
    [MONTHS_FILL_START - 20, MONTHS_FILL_START, TIMELINE["beat-07"].from, TIMELINE["beat-07"].from + 40],
    [0, 1, 1, 0],
    clamp,
  );
