...
import { ACCENT, BG, DIM_TEXT, FONT, LINE_INACTIVE, TEXT } from "../../../lib/essay-sdk/palette.ts";
...
export type SplitArgumentSceneProps = {
  ...
  /** Gold marks the correct move the viewer should retain. Use "none" when
   * both columns are case-study explanation rather than guidance. */
...
export const SplitArgumentScene: React.FC<SplitArgumentSceneProps> = ({
  ...
  const footnoteShift =
    footnoteRange && footnoteStartFrame !== undefined
      ...
      : 0;
  const leftLabelColor = accentSide === "left" ? ACCENT : DIM_TEXT;
  const leftBodyColor = accentSide === "left" ? ACCENT : TEXT;
  const rightLabelColor = accentSide === "right" ? ACCENT : DIM_TEXT;
  const rightBodyColor = accentSide === "right" ? ACCENT : TEXT;
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "center", gap: 70 }}>
        <div style={{ flex: 1, textAlign: "right", opacity: leftOpacity, translate: `${leftShift}px 0px` }}>
          ...
          <div style={{ fontFamily: FONT, fontSize: 30, fontWeight: 700, lineHeight: 1.4, color: leftBodyColor }}>
            {leftText}
          </div>
        </div>
        ...
        <div style={{ flex: 1, opacity: rightOpacity, translate: `${-rightShift}px 0px` }}>
          ...
          <div style={{ fontFamily: FONT, fontSize: 30, fontWeight: 700, lineHeight: 1.4, color: rightBodyColor }}>
            {rightText}
          </div>
        </div>
      </div>
