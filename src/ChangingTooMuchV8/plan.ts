...
const scenes: Scene[] = Object.values(BEATS).filter(beat => beat.id !== 'beat-07').map(beat => {
  ...
  return {
    id: beat.id, duration: audioDuration + (beat.id === 'beat-00' ? 30 : 90), narration: words,
    audio: { src: `voiceover/ChangingTooMuchV2/${beat.id}.mp3`, duration: audioDuration },
    groups: [], items: [], transfers: [], topics: [], takeaways: [], textMotion: 'rise',
  };
});
...
scene('beat-01').relationships = [{from: 'head', to: 'team', start: 30, end: scene('beat-01').duration}];
...
Object.assign(scene('beat-02'), {groups: [head, team], items: [migration, bugs, {...meetings, owner: 'head'}]});
scene('beat-02').relationships = [{from: 'head', to: 'team', start: 0, end: scene('beat-02').duration}];
scene('beat-02').transfers = [{item: 'meetings', to: 'team', start: cue('beat-02', 'asked'), end: cue('beat-02', 'rebuild') + 60}];
...
for (const id of ['beat-03', 'beat-04', 'beat-05', 'beat-06']) {
...
scene('beat-03').items = [
  {id: 'which', label: 'Which meeting?', kind: 'question', owner: 'team', revealAt: cue('beat-03', 'Which')},
  {id: 'report', label: 'What goes in the report?', kind: 'question', owner: 'team', revealAt: cue('beat-03', 'What')},
  {id: 'decision', label: 'When is the decision made?', kind: 'question', owner: 'team', revealAt: cue('beat-03', 'Did')},
];
scene('beat-03').topics = [
  {group: 'team', item: 'which', start: cue('beat-03', 'Which'), end: cue('beat-03', 'What')},
  {group: 'team', item: 'report', start: cue('beat-03', 'What'), end: cue('beat-03', 'Did')},
  {group: 'team', item: 'decision', start: cue('beat-03', 'Did'), end: scene('beat-03').duration},
];
for (const id of ['beat-04']) {
...
// The takeaway phrase is gold from its entrance, never recolored mid-sentence.
scene('beat-09').takeaways = [{phrase: 'main quest', start: 0, end: scene('beat-09').duration}];

scene('beat-10').takeaways = [{text: true, start: 0, end: scene('beat-10').duration}];

export const movie = compileEssay({fps, scenes});
// Exact spoken script, derived from the same approved timing data as playback.
export const script = scenes.map(s => ({id: s.id, text: s.narration.map(w => w.text).join(' ')}));
