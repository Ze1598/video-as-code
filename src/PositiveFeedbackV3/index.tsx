...
import { CtaScene, HookScene, ReinforcementScene, VerdictScene, Wipe } from "./Scenes";
...
export const FPS_V3 = FPS;
export const POSITIVE_FEEDBACK_V3_DURATION = TOTAL_DURATION;
...
// CTA) held for 2.5 seconds after its last word. Beats 1-6 and 9 play out
// over the persistent diagram; Beats 7 and 8 wipe to dedicated full-screen
// scenes because their content — a two-sided argument, a list of three
// things — is better shown as itself than narrated over the diagram.
export const PositiveFeedbackV3: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {BEAT_ORDER.map((id) => (
        <Sequence key={id} from={TIMELINE[id].from} durationInFrames={TIMELINE[id].duration} name={id}>
          <Audio src={staticFile(`voiceover/PositiveFeedbackV3/${id}.mp3`)} />
        </Sequence>
      ))}
